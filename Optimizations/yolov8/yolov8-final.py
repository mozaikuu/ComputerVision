...
# Define model paths
BASE_MODEL = "yolov8n-cls.pt"  # base pretrained model
TRAINED_MODEL_DIR = os.path.join(os.path.dirname(BASE_DIR), 'trained_models')
os.makedirs(TRAINED_MODEL_DIR, exist_ok=True)
TRAINED_MODEL_PATH = os.path.join(TRAINED_MODEL_DIR, 'pneumonia_classifier.pt')

# Function to check if a trained model exists
def get_model():
    if os.path.exists(TRAINED_MODEL_PATH):
        print(f"Loading existing trained model from: {TRAINED_MODEL_PATH}")
        return YOLO(TRAINED_MODEL_PATH)
    else:
        print(f"No trained model found. Loading base model and will train: {BASE_MODEL}")
        return YOLO(BASE_MODEL)

# Load the appropriate model
model = get_model()

# Train the model only if we're using the base model
if not os.path.exists(TRAINED_MODEL_PATH):
    print("Training the model...")
    results = model.train(data=SPLIT_DIR, epochs=5, imgsz=IMG_SIZE)
    
    # Save the trained model
    print(f"Saving trained model to: {TRAINED_MODEL_PATH}")
    # Copy the best model from the training run to our designated location
    best_model_path = os.path.join(os.getcwd(), 'runs', 'classify', 'train', 'weights', 'best.pt')
    ...
    # You can still access the last training results if needed
    results_dir = os.path.join(os.getcwd(), 'runs')
    if os.path.exists(results_dir):
        print(f"Previous training results available at: {results_dir}")
